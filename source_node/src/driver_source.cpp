...
EncoderSource::EncoderSource(){
    ...
    param_server.GetParam("driver_source_node");
    logger.init_logger(param_server.car_id);
    if(LoadConfig(path)){
        node_state = State::HAVE_CONFIG;
    }else{
        node_error = Error::NO_ERROR;
        logger.WARNINFO(param_server.car_id,"encoder_node: Config failed");
    }
...
bool EncoderSource::LoadConfig(std::string file){
    logger.DEBUGINFO(param_server.car_id,"encoder_node: load config");
...
void EncoderSource::UpdateState(){
    ...
    logger.DEBUGINFO(param_server.car_id,"encoder_node: state thread start");
...
State EncoderSource::Start(){
    logger.DEBUGINFO(param_server.car_id,"encoder source running");
    return State::RUNNING;
}
//Stop需要reset参数
State EncoderSource::Stop(){
    logger.DEBUGINFO(param_server.car_id,"encoder source stop");
    return State::STOP;
}
//退出需要清理线程
State EncoderSource::Exit(){
    logger.DEBUGINFO(param_server.car_id,"encoder source exit");
...
State EncoderSource::Pause(){
    if(node_state!=State::RUNNING)
        return node_state;
    logger.DEBUGINFO(param_server.car_id,"encoder source pause");
    return State::PAUSED;
}
State EncoderSource::Resume(){
    if(node_state!=State::PAUSED)
        return node_state;
    logger.DEBUGINFO(param_server.car_id,"encoder source resume");
    return State::RUNNING;
}

void EncoderSource::CmdCallback(const robot_msgs::CmdConstPtr &msg){
    logger.DEBUGINFO(param_server.car_id,"encoder source get command");
    switch(msg->cmd){
