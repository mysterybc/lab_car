...
{
    param_server.GetParam("build_up_nohost_node");
    ...
    robot_pose_sub = nh.subscribe("odom",10,&BuildUpTask::OnNewPose,this);
...
bool BuildUpTask::MakePlanWithoutExcute(geometry_msgs::Pose goal){
    ...
    if(make_plan_client.call(get_plan)){
        ...
        logger.WARNINFO(param_server.car_id,"fail to call path");
        return false;
    }
    //report path
    ...
    if(report_path_client.call(report_path)){
       ...
       logger.WARNINFO(param_server.car_id,"fail to report path");
        return false;
    }
    return true;
}
...
void BuildUpTask::BuildUpExcuteCB(const robot_msgs::BuildUpGoalConstPtr &goal){
    ...
    if(separate_goal_client.call(separate_goal)){
        ...
        logger.WARNINFO(param_server.car_id,"separate goal failed!");
        ResetTask();
        return ;
    }
    ros::spinOnce();
    logger.DEBUGINFO(param_server.car_id,"build up target position is [%f,%f]",param_server.car_id,goal_point.position.x,goal_point.position.y);
    ...
    while(ros::ok() && build_up_action.isActive()){
        ...
        if(task_state.status == actionlib_msgs::GoalStatus::SUCCEEDED){
            ...
            result.succeed = false;
            logger.WARNINFO(param_server.car_id,"build up task error");
            build_up_action.setAborted(result,"task failed");
            break;
        }
        else{
