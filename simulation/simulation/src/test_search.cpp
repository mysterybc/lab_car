...
TaskRealize::TaskRealize(){
    search_action_client = new actionlib::SimpleActionClient<robot_msgs::SearchAction>(nh,"search_action",true);
...
int main(int argc,char** argv){
    ...
    ros::NodeHandle nh;
    TaskRealize task_realize;
    ros::Rate loop(20);
    while(ros::ok()){
        if(task_realize.search_goal.area.size() == 4){
            task_realize.Search();
            task_realize.search_goal.area.clear();
        }
        ros::spinOnce();
        loop.sleep();
    }
