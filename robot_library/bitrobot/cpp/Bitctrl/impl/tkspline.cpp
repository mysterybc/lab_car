...
inline void tmpremove(
    std::vector<double>& x,
    std::vector<double>& y,
    int k, int n = 1)
{
    x.erase(x.begin() + k, x.begin() + k + n);
    y.erase(y.begin() + k, y.begin() + k + n);
}
                        ...
                        const std::vector<double>& y, bool cubic_spline)
{
    ...
    // ADD a loop to remove duplicates
    for (size_t i = 0; i < m_x.size() - 1; ++i){
        int nduplicate = 0;
        for (size_t j = i + 1; j < m_x.size(); ++j){
            if (m_x[j] != m_x[i]) break;
            nduplicate++;
        }
        if (nduplicate != 0)
            tmpremove(m_x, m_y, i + 1, nduplicate);
    }
