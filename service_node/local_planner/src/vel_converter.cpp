...
class VelConverter {
 public:
  VelConverter() : new_cmd_acc_(false), begin_(false) {
    ...
    cmd_sub_ = cmd_handle_.subscribe<robot_msgs::TwistAccel>("/cmd_vel_acc", 100, boost::bind(&VelConverter::VelCallback, this, _1));
  }
  void VelCallback(const robot_msgs::TwistAccel::ConstPtr& msg);
  void UpdateVel();
...
void VelConverter::VelCallback(const robot_msgs::TwistAccel::ConstPtr& twist_acc_msg)
{
  if (!begin_) {
    begin_ = true;
  }
