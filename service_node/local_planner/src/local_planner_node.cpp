...
LocalPlannerNode::LocalPlannerNode() :
    ...
    initialized_(false), node_state_(NodeState::IDLE),
    node_error_info_(ErrorCode::OK), max_error_(5),
...
ErrorInfo LocalPlannerNode::Init() {
  ROS_INFO("local planner start");
  LocalAlgorithms local_algorithms;
  std::string full_path = ros::package::getPath("service_node") + "/local_planner/config/local_planner.prototxt";
  ReadProtoFromTextFile(full_path.c_str(), &local_algorithms);
  if (&local_algorithms == nullptr) {
    return ErrorInfo(ErrorCode::LP_INITILIZATION_ERROR,
  ...
  std::string map_path = ros::package::getPath("costmap_node") + \
      "/config/costmap_parameter_config_for_local_plan.yaml";
  local_cost_ = std::make_shared<costmap_node::CostmapInterface>("local_costmap",
                                                                          *tf_,
                                                                          map_path.c_str());
  local_planner_ = AlgorithmFactory<LocalPlannerBase>::CreateAlgorithm(selected_algorithm_);
  ...
  vel_pub_ = local_planner_nh_.advertise<robot_msgs::TwistAccel>("/cmd_vel_acc", 5);

  return ErrorInfo(ErrorCode::OK);
}

void LocalPlannerNode::ExcuteCB(const robot_msgs::LocalPlannerGoal::ConstPtr &command) {
  ErrorInfo error_info = GetErrorInfo();
  NodeState node_state = GetNodeState();

  if (node_state == NodeState::FAILURE) {
    robot_msgs::LocalPlannerFeedback feedback;
    robot_msgs::LocalPlannerResult result;
...
void LocalPlannerNode::Loop() {
  ...
  ErrorInfo error_info = local_planner_->Initialize(local_cost_, tf_, visual_);
  if (error_info.IsOK()) {
  ...
  while (GetNodeState() == NodeState::RUNNING) {
    ...
    ErrorInfo error_info = local_planner_->ComputeVelocityCommands(cmd_vel_);
    ...
    if (error_info.IsOK()) {
      ...
      error_info =  ErrorInfo(ErrorCode::LP_MAX_ERROR_FAILURE, "over max error.");
      SetNodeState(NodeState::FAILURE);
    } else {
      error_count++;
...
void LocalPlannerNode::SetErrorInfo(const ErrorInfo error_info) {
  std::lock_guard<std::mutex> guard(node_error_info_mtx_);
  node_error_info_ = error_info;
}

void LocalPlannerNode::SetNodeState(const NodeState& node_state) {
  std::lock_guard<std::mutex> guard(node_state_mtx_);
  node_state_ = node_state;
}

NodeState LocalPlannerNode::GetNodeState() {
  std::lock_guard<std::mutex> guard(node_state_mtx_);
  return node_state_;
}

ErrorInfo LocalPlannerNode::GetErrorInfo() {
  std::lock_guard<std::mutex> guard(node_error_info_mtx_);
...
void LocalPlannerNode::AlgorithmCB(const ErrorInfo &algorithm_error_info) {
