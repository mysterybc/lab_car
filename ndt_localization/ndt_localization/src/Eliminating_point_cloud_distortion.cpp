...
//计算局部坐标系下点云中的点相对第一个开始点的由于加减速运动产生的位移畸变 平移加旋转
...
//计算局部坐标系下点云中的点相对第一个开始点由于加减速产生的的速度畸变（增量）  速度畸变
void VeloToStartIMU()
{
...
void velodyne_points_cb(const sensor_msgs::PointCloud2::ConstPtr& laserCloudMsg)
{

  pcl::fromROSMsg(*laserCloudMsg, *cur_cloud_Ptr);

  //记录每个scan有曲率的点的开始和结束索引
  std::vector<int> scanStartInd(N_SCANS, 0);  //每一线有曲率的点开始索引scan_vector(16) all 0
  std::vector<int> scanEndInd(N_SCANS, 0);    //每一线有曲率的点结束索引scan_vector(16) all 0
...
void imu_data_cb(const sensor_msgs::Imu::ConstPtr& imuIn)
{
  ...
  //减去重力的影响,求出xyz方向的加速度实际值，并进行坐标轴交换，统一到z轴向前,x轴向左，y轴向上的右手坐标系, 交换过后RPY对应fixed axes ZXY(RPY---ZXY)。Now R = Ry(yaw)*Rx(pitch)*Rz(roll).
...
int main (int argc, char** argv)
{
  ...
  ros::Subscriber sub_Velodyne = nh.subscribe ("/kitti/velo/pointcloud", 1, velodyne_points_cb);
  ros::Subscriber sub_Imu = nh.subscribe<sensor_msgs::Imu> ("/kitti/oxts/imu", 50, imu_data_cb);
  ...
  // cur_T = Eigen::Matrix4f::Identity();
  // last_T = Eigen::Matrix4f::Identity();
